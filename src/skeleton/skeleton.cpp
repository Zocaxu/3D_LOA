...
namespace cgp
{
	...
	buffer<affine_rt> skeleton_animation_structure::evaluate_local(float t) const
	{
		...
		for(size_t kj=0; kj<N_joint; ++kj)
		{
			...
			skeleton_current[kj].translation = alpha*T0.translation + (1-alpha)*T1.translation;
			skeleton_current[kj].rotation = rotation_transform::lerp(T0.rotation, T1.rotation, alpha);
		}
